...
import { getAllOrderSpas } from 'src/services/order-spa'
...
const Dashboard = () => {
  ...
  const fetchListProductPopular = async () => {
    setLoading(true)
    await getAllPackages({ params: { limit: 5, page: 1, order: 'booking desc' } })
      .then(res => {
        const data = res?.data
        setLoading(false)
        setListProductPopular(data?.packages)
      })
      .catch(e => {
        setLoading(false)
      })
